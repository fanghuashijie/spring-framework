...
public interface TransactionDefinition {
	 ...
	 *
	 * 支持当前交易；如果不存在，请创建一个新的。类似于同名的EJB事务属性。 <p>这通常是事务定义的默认设置，并且通常定义事务同步作用域。
	 *
	 * 支持当前事物，如果当前没有事物，就新建一个事务，这是最常见的选择，也是spring默认的事物传播
	 */
	 ...
	 *
	 * 新建事务，如果当前存在事务，把当前事务挂起，新建的事务和被挂起的事务没有任何关系，是两个独立的事务，
	 ...
	 *
	 * 如果一个活动的事务存在，则运行在一个嵌套的事务中，如果没有活动事务，则按 PROPAGATION_REQUIRED 属性执行，它使用了
	 * 一个单独的事务，这个事务拥有多个可以回滚的保存点，内部事务的回滚不会对外部事务造成影响，它只对 DataSourceTransactionManager 事务管理器 起效。
	 */
	 ...
	 * 指示防止脏读，不可重复读和幻像读
	 * 串行化读，事务只能一个一个执行，避免了脏读，不可重复读，幻读、执行效率慢，使用慎重
	 */
